...
async function loadGeneralStats() {
    try {
        ...
        if (data.length > 1) {
            ...
            // A = Comunidade, B = Troféus Atuais, C = Troféus Mês Passado, D = Ganho de Troféus
            updateGeneralStatsDisplay({
                currentTrophies: data[1][1] || 'N/A', // Coluna B - Troféus Atuais
                lastMonthTrophies: data[1][2] || 'N/A', // Coluna C - Troféus Mês Passado
                trophyIncrease: data[1][3] || 'N/A', // Coluna D - Ganho de Troféus
                trophyGrowth: data[1][4] || 'N/A' // Coluna E - Ganhos Percentuais
            });
        }
    } catch (error) {
...
function updateGeneralStatsDisplay(stats) {
    ...
    document.getElementById('trophy-growth').textContent = formatPercentage(stats.trophyGrowth);
}
...
async function loadEvents() {
    ...
    try {
        // Buscar dados da segunda divisão usando GID correto
        const valyrianUrl = `https://docs.google.com/spreadsheets/d/${CONFIG.EVENTS_SPREADSHEET_ID}/gviz/tq?tqx=out:csv&gid=2132978233`;
        const valyrianResponse = await fetch(valyrianUrl);
        const valyrianCsv = await valyrianResponse.text();
        const valyrianData = parseCSV(valyrianCsv);
        
        // Buscar dados da terceira divisão (primeira aba)
        const combatData = await fetchGoogleSheetData(CONFIG.EVENTS_SPREADSHEET_ID, 'Sheet1');
        
        console.log('Valyrian data loaded:', valyrianData);
        console.log('Combat data loaded:', combatData);
...
function createEventCard(title, data, headers) {
    ...
    // Skip header row and show all players
    for (let i = 1; i < data.length; i++) {
        const row = data[i];
        if (row.length > 0 && row[0] && row[0].toString().trim() !== '') {
            ...
            if (headers.length === 2) {
                ...
                // Valyrian Ascent (2ª Divisão): usar coluna Q (índice 16)
                // Combat Championship (3ª Divisão): usar coluna E (índice 4)
                ...
                // Mostrar dados válidos (incluindo valores numéricos)
                if (playerName.trim() !== '' && score !== undefined && score !== null && score !== '') {
                    tableHTML += `<td>${playerName}</td>`;
                    tableHTML += `<td>${score}</td>`;
                } else if (playerName.trim() !== '') {
                    // Se tem nome mas não tem pontuação, mostrar N/A
                    tableHTML += `<td>${playerName}</td>`;
                    tableHTML += `<td>N/A</td>`;
                } else {
                    continue; // Pular linha se não tiver dados válidos
                }
            } else {
